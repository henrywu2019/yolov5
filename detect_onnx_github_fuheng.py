...
def detect_onnx(official=True, image_path=None):
    ...
    session = onnxruntime.InferenceSession('./models/yolov5s_img-640-640_batch-1_device-cpu_dynamic-True_grid-True.onnx')
    ...
    #plt.imshow(resized)
    #resized.save("bus_lbox.jpg")
    #plt.savefig("bus_lbox.jpg")

    #img_in = np.transpose(resized, (0, 1, 2)).astype(np.float32)  # HWC -> CHW
    #img_in = letterbox(img_in, 640, stride=32)[0]
    ...
    img_in = img/255.0  # 0 - 255 to 0.0 - 1.0
    if img_in.ndimension() == 3:
        img_in = img_in.unsqueeze(0)

    img_in = img_in.cpu().detach().numpy()
    print("Shape of the image input shape: ", img_in.shape)

    # inference
    preds = torch.load('torch_preds.t')
    ...
    if official and len(outputs) == 4:   # model.model[-1].export = boolean ---> True:3 False:4
        ...
        batch_detections = non_max_suppression(batch_detections, conf_thres=0.25, iou_thres=0.45, agnostic=False)
    else:
        ...
        for index, out in enumerate(outputs):
            ...
            sgm = torch.sigmoid(out[..., 0])
            pred_boxes[..., 0] = (sgm * 2.0 - 0.5 + grid_x) * stride_w  # cx
            sgm = torch.sigmoid(out[..., 1])
            pred_boxes[..., 1] = (sgm * 2.0 - 0.5 + grid_y) * stride_h  # cy
            sgm = torch.sigmoid(out[..., 2:4])
            pred_boxes[..., 2:4] = ( sgm * 2) ** 2 * anchor_grid[index]  # wh
